...
def parameter_parser():
    ...
    # Data Parameters
    parser.add_argument("--train-file", nargs="?", default="data/flipkart_train_.json", help="Training data.")
    parser.add_argument("--validation-file", nargs="?", default="data/flipkart_val_.json", help="Validation data.")
    parser.add_argument("--test-file", nargs="?", default="data/flipkart_test_.json", help="Testing data.")
    parser.add_argument("--metadata-file", nargs="?", default="data/metadata.tsv",
                        help="Metadata file for embedding visualization.")
    parser.add_argument("--word2vec-file", nargs="?", default="data/word2vec_100/word2vec_100.kv",
                        help="Word2vec file for embedding characters (the dim need to be the same as embedding dim).")

    # Model Hyperparameters
    parser.add_argument("--pad-seq-len", type=int, default=200, help="Padding sequence length. (depends on the data)")
    ...
    parser.add_argument("--num-classes-list", type=list, default=[265, 217, 875, 2348],
                        help="Each number of labels in hierarchical structure. (depends on the task)")
    parser.add_argument("--total-classes", type=int, default=3705, help="Total number of labels. (depends on the task)")
